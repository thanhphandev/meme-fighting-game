...
export class Fighter {
  constructor(
    ...
    if (image) {
      ...
      // Handle custom assets logic (http urls or data URIs)
      if (this.charData.asset.startsWith('http') || this.charData.asset.startsWith('data:')) {
          this.image.crossOrigin = 'anonymous'; // For external HTTP images (like pollinations)
          this.image.src = this.charData.asset;
      } else {
    ...
    this.attackBox = {
      x: 0,
      y: 0,
      width: abConfig.width,
      height: abConfig.height,
    };
    this.input = {
      left: false,
      right: false,
  ...
  setState(stateName) {
    ...
    this.frameX = 0;
    this.maxFrames = this.charData.frameCounts
      ? this.charData.frameCounts[stateName] || 6
      : 6;
    this.hasHit = false;
  }
  ...
  update(deltaTime, opponent, onHit, spawnParticle, spawnProjectile) {
    ...
    if (this.state === "hit" && layout) {
      ...
      if (this.frameX === 3)
        this.checkAttackCollision(opponent, onHit, spawnParticle);

      if (
        this.hasHit &&
        this.frameX >= 4 &&
        (this.inputBuffer?.attack || this.input.attack) &&
        this.stamina >= CONFIG.attackCost
      ) {
        this.setState("idle");
        this.setState("attack");
  ...
  takeDamage(amount, direction, spawnParticle, stunDuration = null) {
    if (this.isInvincible || this.state === "roll" || this.invulnTimer > 0)
      return;
  ...
  useSkill(onHit, spawnProjectile) {
    ...
    if (skill.type === "projectile") {
      ...
      const spawnX =
        this.x + (this.facingRight ? this.width * 0.8 : this.width * 0.2);
      ...
      this.velocityX = this.facingRight
        ? config.speed || 30
        : -(config.speed || 30);
    } else {
  ...
  handleSkillUpdate(deltaTime, opponent, onHit) {
    ...
    if (skill.type === "buff") {
      ...
      if (config.scale)
        this.scale = 1 + (config.scale - 1) * Math.sin(Date.now() / 100);
    } else if (skill.type === "aoe") {
      // Tick damage logic
  ...
  updateAnimation(deltaTime, stopAtEnd = false) {
    ...
    this.maxFrames = this.charData.frameCounts
      ? this.charData.frameCounts[this.state] || 6
      : 6;
  ...
  draw() {
    ...
    // Apply grayscale filter if health is low or dead
    const hpPercent = this.health / CONFIG.baseHp;
    if (this.isDead) {
      this.ctx.filter = "grayscale(100%) brightness(50%) contrast(150%)";
    } else if (hpPercent <= 0.25) {
      // Blink (chớp chớp) effect for low health
